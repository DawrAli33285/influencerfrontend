...
export default function ActiveBids({ state, loading }) {
  ...
  {state?.market?.length > 0 ? state?.market?.map((val, i) => {
    return (
      <div className="bg-white flex-shrink-0 w-[100%] sm:w-[100%] md:w-[30%] lg:w-[30%] flex flex-col gap-[20px] rounded-[20px] relative">
        <img
          src={val?.photos[0]
            ? val.photos[0].replace('http://localhost:5000', BASE_URL)
            : img}
          alt="cardimg"
          className="rounded-[10px] w-full h-[489px] object-cover"
        />
        <div className="absolute bg-[#0000003d] p-[20px] w-full h-full flex flex-col gap-[5px] rounded-[20px] justify-end">
          <p className="text-white lg:text-[1.25rem] text-[1rem] font-bold">
            {val?.issuer_id?.user_id?.username}
          </p>
          <p className="lg:text-[0.94rem] text-[0.75rem] text-[#FFFFFFBF]">
            {val?.missions?.length > 0
              ? val?.missions[0]?.description?.slice(0, 50) + "..."
              : `No mission`}
          </p>
          <span className="lg:text-[1.25rem] text-[1rem] font-bold text-white">
